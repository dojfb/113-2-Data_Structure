...
public:
    Polynomial() : length(0), coef(nullptr) {}  // 無輸入初始化

    Polynomial(int len, float c[][2]) { //利用動態分配記憶體分配客製化記憶體空間儲存係數陣列，並初始化
        length = len;
        coef = new float*[length];  //宣告指標陣列儲存各列的指標

        for (int i = 0; i < length; i++) {  //將輸入陣列(c)的值搬到陣列
            coef[i] = new float[2]; //分配記憶體給各指標陣列元素
            coef[i][0] = c[i][0];   //給值
            coef[i][1] = c[i][1];
        }
    }

    // 為防止物件指向相同**coef位址，需定義=運算
    Polynomial& operator=(const Polynomial& other) {
        if (this != &other) {
            ...
            // Allocate new memory
            length = other.length;
            coef = new float*[length];

            for (int i = 0; i < length; i++) {
                coef[i] = new float[2];
                coef[i][0] = other.coef[i][0];
                coef[i][1] = other.coef[i][1];
            }
        }
        return *this;
    }

    ~Polynomial() { //物件被刪除前呼叫解構子釋放記憶體
        for (int i = 0; i < length; i++) {  //釋放各行的記憶體
            delete[] coef[i];
        }
        delete[] coef;  //釋放指向各行的指標記憶體
    }
