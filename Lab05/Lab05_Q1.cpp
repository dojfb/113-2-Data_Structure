...
void playerTurn(Player* player, Queue& cardDeck) {// 玩家回合
    ...
    while (player->score < 21 && !cardDeck.isEmpty()) {// 玩家小於21點且牌堆不為空
        ...
        if (choice == 'h') {//如果玩家選擇抽牌
            //提示:從牌堆中取出一張牌，然後從牌堆中移除這張牌
            //注意:卡牌變數的類型是Card，並且使用front抽牌
        	newCard = cardDeck.front();
        	cardDeck.dequeue();
            if (newCard.rank == "A"){   //如果抽到A
                char ch2ten;
                cout << player->name << "是否將A當11 ? (y/n)" << endl;  //問玩家是否當11
                cin >> ch2ten;
                if(ch2ten == 'y'){
                    cout << player->name << "把A當11" << endl;          //是玩家分數+11
                    player->score += 11;//計算A點數
                }else{
                    cout << player->name << "把A當1" << endl;           //不是玩家分數+1
