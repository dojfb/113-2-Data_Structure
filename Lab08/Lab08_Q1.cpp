...
private:
    ...
    TreeNode* deleteNode(TreeNode* node, int val) {
        ...
        //如果值小於當前節點，到左子樹去找同值節點，並把修改後的子樹根節點接上
        if (val < node->data) {
            node->left = deleteNode(node->left, val);//讓當前左子樹根節點指向修改後的子樹
        //如果值小於當前節點，到右子樹去找同值節點，並把修改後的子樹根節點接上
        } else if (val > node->data) {
            node->right = deleteNode(node->right, val);//讓當前右子樹根節點指向修改後的子樹
        } else {
            //同值節點為葉節點
            ...
            //同值節點無左子樹
                ...
                //把同值節點刪除，並把它的子樹接上父節點
                TreeNode* t = node->right;  //建立暫時指標儲存node的右子樹根指標
                delete node;                //刪除同值節點
                return t;                   //把右子樹指標值回傳
            //同值節點無右子樹
            } else if (node->right == nullptr) {
                //把同值節點刪除，並把它的子樹接上父節點
                TreeNode* t = node->left;   //建立暫時指標儲存node的左子樹根指標
                delete node;                //刪除同值節點
                return t;                   //把左子樹指標值回傳
            } else {
                // 兩個子節點都存在
                TreeNode* minNode = findMin(node->right);   //先找出右子樹的最小節點
                node->data = minNode->data;                 //把值給到需要刪除的節點裡替換
                node->right = deleteNode(node->right, minNode->data);//把原本的最小節點刪掉
            }
        }
        //把修改後的子樹指標回傳，讓父節點接上
        return node;
