...
public:
    ...
    void heapify(int i) {// 堆化函式（確保以 i 為根的子樹符合Min Heap性質）
        ...
        // 遞迴處理受影響的子樹
        if (smallest != i) {
            swap(heap[smallest], heap[i]);   // 把子樹最小值換到根
            heapify(smallest);               // 確保換完後的子樹符合Min Heap性質
        }
...
int main() {
    // 從文件讀取輸入元素
    string filename = "C:\\Users\\Dan\\Desktop\\mycode\\data_structure\\input\\input3.txt";//請貼上input檔案的正確路徑
